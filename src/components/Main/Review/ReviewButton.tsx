  ...
  return (
    <View style={{marginTop: 20, marginLeft: 20, marginRight: 20, display: 'flex', flexDirection: 'column'}}>
      ...
      <View style={{width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between',}}>
        <TouchableOpacity 
            style={{ width: '48%', height: 120, display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: 20, borderWidth:4, 
            borderColor: theme.olive, backgroundColor: like ? theme.olive : theme.backgroundWhite }}
            onPress={handleClick}>
            <FontAwesome5 name="thumbs-up" size={55} color={like ? theme.backgroundWhite : theme.olive} />
        </TouchableOpacity>
        <TouchableOpacity 
            style={{ width: '48%', height: 120, display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: 20, borderWidth:4, 
            borderColor: theme.olive, backgroundColor: !like ? theme.olive : theme.backgroundWhite }}
            onPress={handleClick}>
            <FontAwesome5 name="thumbs-down" size={55} color={!like ? theme.backgroundWhite : theme.olive} />
        </TouchableOpacity>
      </View>
    </View>
  )
